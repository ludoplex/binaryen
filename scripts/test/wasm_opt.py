...
def test_wasm_opt():
    ...
    for t in shared.get_tests(shared.get_test_dir('passes'), ['.wast', '.wasm']):
        ...
        passname = base
        passes_file = os.path.join(shared.get_test_dir('passes'), f'{passname}.passes')
        ...
        if 'emit-js-wrapper' in t:
            with open('a.js') as actual:
                shared.fail_if_not_identical_to_file(actual.read(), f'{t}.js')
        if 'emit-spec-wrapper' in t:
            with open('a.wat') as actual:
                shared.fail_if_not_identical_to_file(actual.read(), f'{t}.wat')
    ...
    for t in shared.get_tests(shared.get_test_dir('print'), ['.wast']):
        ...
        expected_file = os.path.join(shared.get_test_dir('print'), f'{wasm}.txt')
        shared.fail_if_not_identical_to_file(actual, expected_file)
        ...
        shared.fail_if_not_identical(
            ...
            open(
                os.path.join(
                    shared.get_test_dir('print'), f'{wasm}.minified.txt'
                )
            )
            .read()
            .strip(),
...
def update_wasm_opt_tests():
    ...
    for t in shared.get_tests(shared.get_test_dir('print'), ['.wast']):
        ...
        print(cmd, actual)
        with open(f'{wasm}.txt', 'wb') as o:
            o.write(actual)
        cmd = shared.WASM_OPT + [t, '--print-minified', '-all']
        print('    ', ' '.join(cmd))
        actual = subprocess.check_output(cmd)
        with open(f'{wasm}.minified.txt', 'wb') as o:
            o.write(actual)
    ...
    for t in shared.get_tests(shared.get_test_dir('passes'), ['.wast', '.wasm']):
        ...
        if 'emit-js-wrapper' in t:
            with open('a.js') as i:
                with open(f'{t}.js', 'w') as o:
                    o.write(i.read())
        if 'emit-spec-wrapper' in t:
            with open('a.wat') as i:
                with open(f'{t}.wat', 'w') as o:
                    o.write(i.read())
